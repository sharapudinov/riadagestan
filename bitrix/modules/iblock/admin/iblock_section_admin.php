...
else
{
	if (isset($_REQUEST["apply_filter"]))
	{
		$find_section_section = $arFilter["SECTION_ID"] =
			(is_null($arFilter["SECTION_ID"]) ? 0 : intval($arFilter["SECTION_ID"]));
	}
	else
	{
...
$rsData->NavStart();
$lAdmin->SetNavigationParams($rsData, array("BASE_LINK" => $selfFolderUrl."cat_section_admin.php"));
$arRows = array();
...
$lAdmin->setContextSettings(array("pagePath" => $selfFolderUrl.CIBlock::GetAdminSectionListScriptName($IBLOCK_ID, array("skip_public" => "Y"))));
