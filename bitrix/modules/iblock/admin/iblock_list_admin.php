...
else
{
	...
	{
		$find_section_section = (is_null($arFilter["SECTION_ID"]) ? 0 : intval($arFilter["SECTION_ID"]));
	}
	else
	{
...
while($arRes = $rsData->NavNext(false))
{
	$sec_list_url = htmlspecialcharsbx($selfFolderUrl.CIBlock::GetAdminSectionListLink(
		$IBLOCK_ID, array('find_section_section'=>$arRes['ID'])));
	$el_edit_url = $selfFolderUrl.CIBlock::GetAdminElementEditLink($IBLOCK_ID, $arRes['ID'],
		array('find_section_section'=>intval($find_section_section), "replace_script_name" => true, "WF"=>"Y"));
	$sec_edit_url =  $selfFolderUrl.CIBlock::GetAdminSectionEditLink($IBLOCK_ID, $arRes['ID'],
	...
	if($arRes['TYPE']=="E")
	{
		...
		elseif($bBizproc)
		{
			$lockStatus = call_user_func(array(ENTITY, "IsDocumentLocked"), $arRes['ID'], "") ? "red" : "green";
		}
		else
		{
			$lockStatus = "";
	...
	if($arRes['TYPE']=="S")
	{
		$bReadOnly = !CIBlockSectionRights::UserHasRightTo($IBLOCK_ID, $arRes['ID'], "section_edit");
		$mainEntityEditPrice = true;
	}
	else
	{
		$bReadOnly = !CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $arRes['ID'], "element_edit");
	...
	if($arRes['TYPE']=="S") // double click moves deeper
	{
		$arRes["PREVIEW_PICTURE"] = $arRes["PICTURE"];
		$row = $lAdmin->AddRow($arRes['TYPE'].$arRes['ID'], $arRes, $sec_list_url, GetMessage("IBLIST_A_LIST"));
	}
	else // in case of element take his action
	{
		$row = $lAdmin->AddRow($arRes['TYPE'].$arRes['ID'], $arRes, $el_edit_url);
		$arElemID[] = $arRes['ID'];
	}
	$arRows[$arRes['TYPE'].$arRes['ID']] = $row;
	...
	if($arRes['TYPE']=="S")
	{
		if(array_key_exists("ELEMENT_CNT", $arVisibleColumnsMap))
		{
			$row->AddViewField("ELEMENT_CNT", $arRes['ELEMENT_CNT'].'('.(int)CIBlockSection::GetSectionElementsCount($arRes['ID'], array("CNT_ALL"=>"Y")).')');
		}

		if(array_key_exists("SECTION_CNT", $arVisibleColumnsMap))
		{
			$row->AddViewField("SECTION_CNT", " ".(int)(CIBlockSection::GetCount(array("IBLOCK_ID"=>$IBLOCK_ID, "SECTION_ID"=>$arRes['ID']))));
		}
	}
	...
	if($arRes['TYPE']=="E")
	{
		...
		if (!$bReadOnly)
		{
			...
			if (array_key_exists("PREVIEW_TEXT", $arVisibleColumnsMap))
			{
				$sHTML = '<input type="radio" name="FIELDS['.$arRes['TYPE'].$arRes['ID'].'][PREVIEW_TEXT_TYPE]" value="text" id="'.$arRes['TYPE'].$arRes['ID'].'PREVIEWtext"';
				if($arRes["PREVIEW_TEXT_TYPE"]!="html")
					$sHTML .= ' checked';
				$sHTML .= '><label for="'.$arRes['TYPE'].$arRes['ID'].'PREVIEWtext">text</label> /';
				$sHTML .= '<input type="radio" name="FIELDS['.$arRes['TYPE'].$arRes['ID'].'][PREVIEW_TEXT_TYPE]" value="html" id="'.$arRes['TYPE'].$arRes['ID'].'PREVIEWhtml"';
				if($arRes["PREVIEW_TEXT_TYPE"]=="html")
					$sHTML .= ' checked';
				$sHTML .= '><label for="'.$arRes['TYPE'].$arRes['ID'].'PREVIEWhtml">html</label><br>';
				$sHTML .= '<textarea rows="10" cols="50" name="FIELDS['.$arRes['TYPE'].$arRes['ID'].'][PREVIEW_TEXT]">'.htmlspecialcharsex($arRes["PREVIEW_TEXT"]).'</textarea>';
				$row->AddEditField("PREVIEW_TEXT", $sHTML);
			}
			if (array_key_exists("DETAIL_TEXT", $arVisibleColumnsMap))
			{
				$sHTML = '<input type="radio" name="FIELDS['.$arRes['TYPE'].$arRes['ID'].'][DETAIL_TEXT_TYPE]" value="text" id="'.$arRes['TYPE'].$arRes['ID'].'DETAILtext"';
				if($arRes["DETAIL_TEXT_TYPE"]!="html")
					$sHTML .= ' checked';
				$sHTML .= '><label for="'.$arRes['TYPE'].$arRes['ID'].'DETAILtext">text</label> /';
				$sHTML .= '<input type="radio" name="FIELDS['.$arRes['TYPE'].$arRes['ID'].'][DETAIL_TEXT_TYPE]" value="html" id="'.$arRes['TYPE'].$arRes['ID'].'DETAILhtml"';
				if($arRes["DETAIL_TEXT_TYPE"]=="html")
					$sHTML .= ' checked';
				$sHTML .= '><label for="'.$arRes['TYPE'].$arRes['ID'].'DETAILhtml">html</label><br>';
				$sHTML .= '<textarea rows="10" cols="50" name="FIELDS['.$arRes['TYPE'].$arRes['ID'].'][DETAIL_TEXT]">'.htmlspecialcharsex($arRes["DETAIL_TEXT"]).'</textarea>';
			...
			if (array_key_exists("TAGS", $arVisibleColumnsMap))
			{
				if ($bSearch)
				{
					...
					$row->AddEditField("TAGS", InputTags("FIELDS[".$arRes['TYPE'].$arRes['ID']."][TAGS]", $arRes["TAGS"], $arIBlock["SITE_ID"]));
				}
				else
				{
	...
	$row->AddViewField("ID", '<a href="'.($arRes['TYPE']=="S"?$sec_edit_url:$el_edit_url).'" title="'.GetMessage("IBLIST_A_EDIT").'">'.$arRes['ID'].'</a>');

	$arProperties = array();
	if($arRes['TYPE']=="E" && !empty($arSelectedProps))
	{
		...
		foreach($arSelectedProps as $aProp)
		{
			...
			foreach($arProperties[$aProp["ID"]] as $prop_id => $prop)
			{
				...
				$VALUE_NAME = 'FIELDS['.$arRes['TYPE'].$arRes['ID'].'][PROPERTY_'.$prop['ID'].']['.$prop['PROPERTY_VALUE_ID'].'][VALUE]';
				$DESCR_NAME = 'FIELDS['.$arRes['TYPE'].$arRes['ID'].'][PROPERTY_'.$prop['ID'].']['.$prop['PROPERTY_VALUE_ID'].'][DESCRIPTION]';
				//View part
				...
				if($bUserMultiple)
				{
					...
					{
						$VALUE_NAME = 'FIELDS['.$arRes['TYPE'].$arRes['ID'].'][PROPERTY_'.$prop['ID'].']';
				...
				elseif($prop['PROPERTY_TYPE']=='F' && ($last_property_id != $prop["ID"]))
				{
					if($prop['MULTIPLE'] == "Y")
					{
						...
						foreach($arProperties[$prop["ID"]] as $g_prop)
						{
							$inputName['FIELDS['.$arRes['TYPE'].$arRes['ID'].'][PROPERTY_'.$prop['ID'].']['.$g_prop['PROPERTY_VALUE_ID'].'][VALUE]'] = $g_prop["VALUE"];
						}
						if (class_exists('\Bitrix\Main\UI\FileInput', true))
						{
							$arEditHTML[] = \Bitrix\Main\UI\FileInput::createInstance(array(
									"name" => 'FIELDS['.$arRes['TYPE'].$arRes['ID'].'][PROPERTY_'.$prop['ID'].'][n#IND#]',
						...
						{
							$arEditHTML[] = CFileInput::ShowMultiple($inputName, 'FIELDS['.$arRes['TYPE'].$arRes['ID'].'][PROPERTY_'.$prop['ID'].'][n#IND#]', array(
								"IMAGE" => "Y",
								"PATH" => "Y",
								"FILE_SIZE" => "Y",
								"DIMENSIONS" => "Y",
				...
				elseif($prop['PROPERTY_TYPE']=='E')
				{
					$VALUE_NAME = 'FIELDS['.$arRes['TYPE'].$arRes['ID'].'][PROPERTY_'.$prop['ID'].']['.$prop['PROPERTY_VALUE_ID'].']';
			...
			$table_id = md5($arRes['TYPE'].$arRes['ID'].':'.$aProp['ID']);
			if($aProp["MULTIPLE"] == "Y")
			{
				$VALUE_NAME = 'FIELDS['.$arRes['TYPE'].$arRes['ID'].'][PROPERTY_'.$prop['ID'].'][n0][VALUE]';
				$DESCR_NAME = 'FIELDS['.$arRes['TYPE'].$arRes['ID'].'][PROPERTY_'.$prop['ID'].'][n0][DESCRIPTION]';
				...
				elseif($prop['PROPERTY_TYPE']=='E')
				{
					$VALUE_NAME = 'FIELDS['.$arRes['TYPE'].$arRes['ID'].'][PROPERTY_'.$prop['ID'].'][n0]';
					$fixIBlock = $prop["LINK_IBLOCK_ID"] > 0;
					$windowTableId = 'iblockprop-'.Iblock\PropertyTable::TYPE_ELEMENT.'-'.$prop['ID'].'-'.$prop['LINK_IBLOCK_ID'];
					$arEditHTML[] = '<input type="text" name="'.$VALUE_NAME.'" id="'.$VALUE_NAME.'" value="" size="5">'.
						'<input type="button" value="..." onClick="jsUtils.OpenWindow(\''.$selfFolderUrl.'iblock_element_search.php?lang='.LANGUAGE_ID.'&amp;IBLOCK_ID='.$prop["LINK_IBLOCK_ID"].'&amp;n='.urlencode($VALUE_NAME).($fixIBlock ? '&amp;iblockfix=y' : '').'&amp;tableId='.$windowTableId.'\', 900, 700);">'.
		...
		if (!$bReadOnly)
		{
			if ($boolEditPrice && $boolCatalogPrice)
			{
				...
				if ($boolCatalogPurchasInfo)
				{
					...
					{
						$editFieldCode = '<input type="hidden" name="FIELDS_OLD[E'.$arRes['ID'].'][CATALOG_PURCHASING_PRICE]" value="'.$row->arRes['CATALOG_PURCHASING_PRICE'].'">';
						$editFieldCode .= '<input type="hidden" name="FIELDS_OLD[E'.$arRes['ID'].'][CATALOG_PURCHASING_CURRENCY]" value="'.$row->arRes['CATALOG_PURCHASING_CURRENCY'].'">';
						$editFieldCode .= '<input type="text" size="5" name="FIELDS[E'.$arRes['ID'].'][CATALOG_PURCHASING_PRICE]" value="'.$row->arRes['CATALOG_PURCHASING_PRICE'].'">';
						$editFieldCode .= '<select name="FIELDS[E'.$arRes['ID'].'][CATALOG_PURCHASING_CURRENCY]">';
	...
	if ($bBizproc)
	{
		if ($arRes['TYPE'] == "E")
		{
			$arDocumentStates = CBPDocument::GetDocumentStates(
				...
				array(MODULE_ID, ENTITY, $arRes['ID'])
			);
	...
	if($arRes['ACTIVE'] == "Y")
	{
		$arActive = array(
			...
			"ACTION" => $lAdmin->ActionDoGroup($arRes['TYPE'].$arRes['ID'], "deactivate", $sThisSectionUrl),
			"ONCLICK" => "",
		);
	}
	else
	{
		$arActive = array(
			...
			"ACTION" => $lAdmin->ActionDoGroup($arRes['TYPE'].$arRes['ID'], "activate", $sThisSectionUrl),
			"ONCLICK" => "",
		);
	}
	...
	$clearCounter = array(
		...
		"ACTION" => "if(confirm('".GetMessageJS("IBLIST_A_CLEAR_COUNTER_CONFIRM")."')) ".$lAdmin->ActionDoGroup($arRes['TYPE'].$arRes['ID'], "clear_counter", $sThisSectionUrl),
		"ONCLICK" => ""
	);

	if($arRes['TYPE']=="S")
	{
		if(CIBlockSectionRights::UserHasRightTo($IBLOCK_ID, $arRes['ID'], "section_edit"))
			$arActions[] = array(
				"ICON" => "edit",
		...
		if(CIBlockSectionRights::UserHasRightTo($IBLOCK_ID, $arRes['ID'], "section_delete"))
			$arActions[] = array(
				"ICON" => "delete",
				"TEXT" => GetMessage("MAIN_DELETE"),
				"ACTION" => "if(confirm('".GetMessageJS("IBLOCK_CONFIRM_DEL_MESSAGE")."')) ".$lAdmin->ActionDoGroup($arRes['TYPE'].$arRes['ID'], "delete", $sThisSectionUrl),
			);
	}
	elseif($bWorkFlow)
	{
		if (CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $arRes['ID'], "element_edit_any_wf_status"))
			$STATUS_PERMISSION = 2;
		else
		...
		$arUnLock = Array(
			...
			"ACTION" => "if(confirm('".GetMessageJS("IBLIST_A_UNLOCK_CONFIRM")."')) ".$lAdmin->ActionDoGroup($arRes['TYPE'].$arRes_orig['ID'], "unlock", $sThisSectionUrl),
		);
		...
		else
		{
			 ...
			 */
		if (
				CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $arRes['ID'], "element_edit")
				&& (2 <= $STATUS_PERMISSION)
			)
			{
			...
			if(strlen($arRes['DETAIL_PAGE_URL'])>0)
			{
				...
				elseif (
					...
					&& CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $arRes['ID'], "element_edit")
					&& CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $arRes['ID'], "element_edit_any_wf_status")
				)
				{
					$arActions[] = array("SEPARATOR"=>true);
					$arActions[] = array(
			...
			if (
				CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $arRes['ID'], "element_delete")
				&& (2 <= $STATUS_PERMISSION)
			)
			{
				if (!CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $arRes['ID'], "element_edit_any_wf_status"))
					$intMinPerm = CIBlockElement::WF_GetStatusPermission($arRes["WF_STATUS_ID"], $arRes['ID']);
				if (2 <= $intMinPerm)
				{
					$arActions[] = array("SEPARATOR"=>true);
					$arActions[] = array(
						...
						"ACTION" => "if(confirm('".GetMessageJS('IBLOCK_CONFIRM_DEL_MESSAGE')."')) ".$lAdmin->ActionDoGroup($arRes['TYPE'].$arRes_orig['ID'], "delete", $sThisSectionUrl),
					);
				}
			}
		}
	...
	elseif($bBizproc)
	{
		...
		if ($bStartWorkflowPermission)
		{
			$arActions[] = array(
				...
				"ACTION" => $lAdmin->ActionRedirect('iblock_start_bizproc.php?document_id='.$arRes['ID'].'&document_type=iblock_'.$IBLOCK_ID.'&back_url='.urlencode($APPLICATION->GetCurPageParam("", array("mode", "table_id"))).''),
			);
		}
		...
		if ($lockStatus == "red")
		{
			if (CBPDocument::IsAdmin())
			{
				$arActions[] = Array(
					...
					"TEXT" => GetMessage("IBLIST_A_UNLOCK"),
					"TITLE" => GetMessage("IBLIST_A_UNLOCK_ALT"),
					"ACTION" => "if(confirm('".GetMessageJS("IBLIST_A_UNLOCK_CONFIRM")."')) ".$lAdmin->ActionDoGroup($arRes['TYPE'].$arRes['ID'], "unlock", $sThisSectionUrl),
				);
			}
		}
		elseif ($bWritePermission)
		{
			$arActions[] = array(
				...
				"TEXT" => GetMessage("IBLOCK_CHANGE"),
				"DEFAULT" => true,
				"ACTION" => $lAdmin->ActionRedirect(CIBlock::GetAdminElementEditLink($IBLOCK_ID, $arRes['ID'], array(
					'WF' => 'Y',
					'find_section_section' => intval($find_section_section)
				)))
			);
			...
			if(!defined("CATALOG_PRODUCT"))
			{
				$arActions[] = array(
					...
					"TEXT" => GetMessage("IBLIST_A_HIST"),
					"TITLE" => GetMessage("IBLIST_A_HISTORY_ALT"),
					"ACTION" => $lAdmin->ActionRedirect('iblock_bizproc_history.php?document_id='.$arRes['ID'].'&back_url='.urlencode($APPLICATION->GetCurPageParam("", array())).''),
				);
			}
	...
	{
		if (CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $arRes['ID'], "element_edit"))
		{
			$arActions[] = array(
				"ICON" => "edit",
				"DEFAULT" => true,
		...
		if ($boolIBlockElementAdd && CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $arRes['ID'], "element_edit"))
		{
			$arActions[] = array(
				"ICON" => "copy",
				"TEXT" => GetMessage("IBLIST_A_COPY_ELEMENT"),
		...
		if (CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $arRes['ID'], "element_delete"))
		{
			if (!empty($arActions))
				$arActions[] = array("SEPARATOR"=>true);
			$arActions[] = array(
