...
*/
if (!in_array($wizrdTemplateId, array("media")))
	$wizrdTemplateId = "media";
...
if($logo || $logoRetina)
{
	if ($logo)
	{
		...
		@copy($strOldFile, WIZARD_SITE_PATH."include/header/logo.png");
		CFile::Delete($fLogo);
	}
	...
	if($logoRetina)
	{
		...
		@copy($strOldFile, WIZARD_SITE_PATH."include/header/logo_retina.png");
		CFile::Delete($fLogoRetina);
	}

	$retinaPath = file_exists(WIZARD_SITE_ROOT_PATH."include/header/logo_retina.png") ? WIZARD_SITE_PATH."include/logo_retina.png" : "";
	$content = '<img src="'.WIZARD_SITE_DIR.'include/header/logo.png"'.($retinaPath ? ' srcset="'.$retinaPath.' 2x"' : "").'/>';
	___writeToAreasFile(WIZARD_SITE_PATH."include/logo.php", $content);
}

$fLogoFooter = CFile::GetByID($wizard->GetVar("siteLogoFooter"));
$logoFooter = $fLogoFooter->Fetch();
$fLogoFooterRetina = CFile::GetByID($wizard->GetVar("siteLogoFooterRetina"));
$logoFooterRetina = $fLogoFooterRetina->Fetch();
if($logoFooter || $logoFooterRetina)
{
	if ($logoFooter)
	{
		$strOldFile = str_replace("//", "/", WIZARD_SITE_ROOT_PATH."/".(COption::GetOptionString("main", "upload_dir", "upload"))."/".$logoFooter["SUBDIR"]."/".$logoFooter["FILE_NAME"]);
		@copy($strOldFile, WIZARD_SITE_PATH."include/footer/logo.png");
		CFile::Delete($fLogoFooter);
	}

	if($logoFooterRetina)
	{
		$strOldFile = str_replace("//", "/", WIZARD_SITE_ROOT_PATH."/".(COption::GetOptionString("main", "upload_dir", "upload"))."/".$logoFooterRetina["SUBDIR"]."/".$logoFooterRetina["FILE_NAME"]);
		@copy($strOldFile, WIZARD_SITE_PATH."include/footer/logo.png");
		CFile::Delete($fLogoFooterRetina);
	}

	$retinaPath = file_exists(WIZARD_SITE_ROOT_PATH."include/footer/logo_retina.png") ? WIZARD_SITE_PATH."include/footer/logo_retina.png" : "";
	$content = '<img src="'.WIZARD_SITE_DIR.'include/logo_footer.png"'.($retinaPath ? ' srcset="'.$retinaPath.' 2x"' : "").'/>';
	___writeToAreasFile(WIZARD_SITE_PATH."include/footer/logo.php", $content);
}
?>
