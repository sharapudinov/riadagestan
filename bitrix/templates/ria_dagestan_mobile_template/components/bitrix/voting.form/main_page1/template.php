...
<?if (!empty($arResult["QUESTIONS"])):?>
	...
	<form action="<?=POST_FORM_ACTION_URI?>" method="post">
      <div class="zagolovok_fon">
        <h2 class="zagolovok_text">Опрос</h2>
      </div>
	  <div class="mmmvoit">
	...
	<?foreach ($arResult["QUESTIONS"] as $arQuestion):?>
		...
		<p class="vopros"><?=$arQuestion["QUESTION"]?><?if($arQuestion["REQUIRED"]=="Y"){echo "<span class='starrequired'>*</span>";}?></p>
		...
		<?foreach ($arQuestion["ANSWERS"] as $arAnswer):?>
			...
			<?switch ($arAnswer["FIELD_TYPE"]):
				case 0://radio?>
                     <div><label><input <?=$value?> type="radio" name="vote_radio_<?=$arAnswer["QUESTION_ID"]?>" value="<?=$arAnswer["ID"]?>" <?=$arAnswer["~FIELD_PARAM"]?> /><p class="variant"><?=$arAnswer["MESSAGE"]?></p></label> </div>
	...
	<div class="niz">
			<div class="vse_oprosi"><a href="polls-archive.php">Все опросы</a></div>
	</div>
