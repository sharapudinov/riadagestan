...
?>
			<div class="vote-item-question1"><b><?=$arQuestion["QUESTION"]?></b	></div>
			...
			<?foreach ($arQuestion["ANSWERS"] as $arAnswer):?>
				...
				<div class="myvoit">
					<? $percent = round($arAnswer["BAR_PERCENT"] * 0.8); // (100% bar * 0.8) + (20% span counter) = 100% td ?>
						...
						<span class="vote-answer1"><?=($arAnswer["COUNTER"] > 0?'&nbsp;':'')?><?=$arAnswer["COUNTER"]?> (<?=$arAnswer["PERCENT"]?>%)</span>
					&nbsp;<br/><br/>
					<div class="vote-answer-bar" style="width:<?=$percent?>%;background-color:#<?=$arAnswer["COLOR"]?>"></div><br/><br/>
					</div>
					<? if (isset($arResult['GROUP_ANSWERS'][$arAnswer['ID']])): ?>
