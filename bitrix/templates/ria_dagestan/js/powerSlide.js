(function(JQuery){
	...
	JQuery.fn.powerSlide = function(options) {  

		var opt = {
			'width': 708, // Width and height of the images
			'height': 340,
		...
		this.each(function() {        
			...
			-----------------------------------------------*/
			JQuery(this).children().wrapAll('<div class="powerSlide" />');
			var container = JQuery(this).find('.powerSlide');
			...
			wrapper.find('img').each(function(i){
				...
				JQuery(this).wrapAll('<div class="image" />');
				if (title !== undefined) {
						JQuery(this).attr('title', '');
						JQuery(this).after('<p>'+ title +'</p>');
