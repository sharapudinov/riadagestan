...
?>
<div class="catalog-top-banner mb-3 <? echo $templateData['TEMPLATE_CLASS']; ?>" id="<? echo $strContID; ?>">
		...
		foreach ($arResult['ITEMS'] as $key => $arItem)
		{
			...
			<div id="<? echo $strMainID; ?>" class="catalog-top-banner-slide<? echo ($boolFirst ? ' active' : ''); ?>">
				<div class="catalog-top-banner-img-block">
					...
					<a id="<? echo $arItemIDs['PICT']; ?>" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" class="catalog-top-banner-img-element" style="background-image: url(<? echo $arItem['PREVIEW_PICTURE']['SRC']; ?>);" title="<? echo $imgTitle; ?>">
						<?
						if ($showPrice && 'Y' == $arParams['SHOW_DISCOUNT_PERCENT'])
						{
							?>
							<div id="<? echo $arItemIDs['DSC_PERC']; ?>" class="bx_stick_disc right bottom" style="display:<?=($price['PERCENT'] > 0 ? '' : 'none'); ?>;">-<?=$price['PERCENT']; ?>%</div>
							<?
						}
						if ($arItem['LABEL'])
						{
				...
				<div class="catalog-top-banner-info-block">
					...
					{
						?>
						<div class="catalog-top-banner-description"><? echo $arItem['PREVIEW_TEXT']; ?></div>
<!--						<div class="catalog-top-banner-description"><pre>--><?// echo var_dump($arItem); ?><!--</pre></div>-->
						<?
					}
					?>
					<div class="catalog-top-banner-price-container">
						...
						<div class="catalog-top-banner-price-left-block">
							...
							{
								?>
								<a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" class="btn btn-primary btn-lg">
									<?
							...
							else
							{
								...
								{
									?>
									<a id="<? echo $arItemIDs['BUY_LINK']; ?>" href="javascript:void(0)" rel="nofollow" class="btn btn-primary btn-lg">
